...
// 1. Fix missing bracket for getAvatarColor (if missing)
const avatarColorStart = code.indexOf('function getAvatarColor(str) {');
const avatarColorEnd = code.indexOf('function AuthorAvatar(', avatarColorStart);
const getAvatarColorCode = code.substring(avatarColorStart, avatarColorEnd);
if (!getAvatarColorCode.includes('}')) {
  code = code.substring(0, avatarColorEnd) + '}\n\n' + code.substring(avatarColorEnd);
} else {
  // ensure the last non-whitespace char is }
  const trimmed = getAvatarColorCode.trim();
  if (!trimmed.endsWith('}')) {
    code = code.substring(0, avatarColorEnd) + '}\n\n' + code.substring(avatarColorEnd);
  }
}

// 2. Fix ArrowLeft import
...
// 4. Replace Left Pane Style
const leftPaneMarker = '{/* ── Left Pane: Comment & Thread List ── */}';
const leftPaneStart = code.indexOf(leftPaneMarker);
const leftPaneNextDiv = code.indexOf('{/* Controls: Search, Account & Status Filter */}', leftPaneStart);
if (leftPaneStart !== -1 && leftPaneNextDiv !== -1) {
  code = code.substring(0, leftPaneStart) + 
        ...
        >\n          ` + 
         code.substring(leftPaneNextDiv);
} else {
  console.error("Failed to find Left Pane boundaries");
  process.exit(1);
}

// 5. Replace Right Pane
const rightPaneMarker = '{/* ── Right Pane: Thread Detail & Reply Composer ── */}';
const rightPaneStart = code.indexOf(rightPaneMarker);

const selectConversationEnd = 'Select a conversation to start chatting</div>';
const selectConversationIdx = code.indexOf(selectConversationEnd, rightPaneStart);
if (selectConversationIdx === -1) {
  console.error("Failed to find select conversation string");
  process.exit(1);
}
// Find the first )} after Select a conversation to start chatting
const endRightPaneIdx = code.indexOf(')}', selectConversationIdx) + 2; // include )}

if (rightPaneStart === -1 || endRightPaneIdx === -1) {
  console.error("Failed to find Right Pane boundaries");
  process.exit(1);
}
...
code = code.substring(0, rightPaneStart) + newRightPane + code.substring(endRightPaneIdx);

fs.writeFileSync('client/src/pages/SocialInboxPage.jsx', code);
console.log('Successfully completed full reliable replacement');
